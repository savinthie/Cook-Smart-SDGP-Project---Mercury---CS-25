...
allergyCollection=db['allergy_info']
...
def get_output():
    ...
    # found the ingredients of the foodname
        ...
        if (ingredients.find(user) != -1):  # find the index of the user entered allergen
            flash("Allergen is present in the ingredients. Please be cautious while consuming.")
    else:
        flash("Allergen is not present in the ingredients.")
...
def get_output():
    if request.method == 'POST':
        ...
        allergies = request.args.getlist('allergyListElement')
        print(allergies)
...
@cooksmartapp.route('/viewAllergy', methods=['POST'])
def allergy_list():
...
def allergy_list(x):
    ...
    for i in allergy_list:
        if (x.find(i) != -1):  # find the index of the user entered allergen
...
@cooksmartapp.route("/usernotification")
def notification():
     notifications = get_notifications()
    
     return render_template,jsonify("notification.html",notifications)
